...
const stageNames = {preparing:'Preparing investigation',reading_demo:'Reading synthetic logs',searching_current_window:'Searching selected window',searching_baseline:'Building historical baseline',gathering_evidence:'Gathering supporting evidence',analyzing_evidence:'Analyzing evidence',investigating:'Investigating candidates',finished:'Run finished',stopped:'Stopped at checkpoint',failed:'Needs attention'};
...
async function loadSettings(){const data=await api('/api/config');settings=data.config;$('yaml-editor').value=data.yaml;renderConnections();renderSettings();}
...
const formSections=[
  ...
  ['hunts','Hunt limits',[['max_window_hours','Maximum window (hours)','number'],['baseline_days','Historical baseline (days)','number'],['max_candidates','Candidates per hunt','number'],['evidence_per_candidate','Selected events per candidate','number'],['max_followups','Follow-up searches per candidate','number']]]
];
